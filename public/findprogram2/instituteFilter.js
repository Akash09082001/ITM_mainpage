// Function to fetch data from API and then filter based on institute
async function filterDataByInstitute(institute) {
    try {
        // Fetch data from the API
        const response = await fetch('../findprogram2/program.json');
        const data = await response.json();

        // Filter the data based on the selected institute
        const filteredData = data.flatMap(category =>
            category.data.filter(item => item.institute.includes(institute))
        );
...
function displayFilteredData(filteredData) {
    const filteredInstitudeList = document.getElementById('main');

    // Clear previous content
    filteredInstitudeList.innerHTML = '';
    ...
    filteredData.forEach(item => {
        ...
        document.getElementById('main').appendChild(newItem1);
    });
}
